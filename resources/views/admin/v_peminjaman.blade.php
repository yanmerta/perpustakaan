                                    ...
                                    @if (auth()->user()->role == 'admin')
                                        <td>
                                            <button class="btn btn-warning btn-sm"
                                                onclick='editPeminjaman(@json($item))'>Edit</button>
                    ...
                    <div class="modal-body">
                        <div id="divIdBuku" class="mb-2">
                            ...
                            <select name="id_buku" id="id_buku" class="form-control">
                                ...
                                @foreach ($buku as $bk)
                                    <option value="{{ $bk->id_buku }}">{{ $bk->kode_buku }} - {{ $bk->judul }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div id="divKodeBuku" style="display:none;">
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#id_buku').on('change', function() {
                ...
                let kodeBuku = selectedOption.text().split(' - ')[0]; // Ambil bagian sebelum tanda ' - '
                $('#kode_buku').val(kodeBuku);
                $('#divKodeBuku').show(); // Tampilkan kolom kode buku saat buku dipilih
            });
        });
        ...
        function editPeminjaman(data) {
            ...
            $('#kode_buku').val(data.buku.kode_buku);
            $('#divKodeBuku').show();
