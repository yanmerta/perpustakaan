                    ...
                    <table id="pengembalianTable" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @foreach ($pengembalian as $i => $data)
                                <tr>
                                    <td>{{ $i + 1 }}</td>
                                    <td>{{ $data->peminjaman->buku->kode_buku }}</td>
                                    ...
                                    <td>
                                        <button class="btn btn-warning btn-sm"
                                            onclick='editPengembalian(@json($data))'>Edit</button>
                                        <form action="{{ route('pengembalian.destroy', $data->id_pengembalian) }}"
                        ...
                        <div class="mb-2">
                            ...
                            <select name="id_peminjaman" id="id_peminjaman" class="form-select" required>
                                ...
                                @foreach ($peminjaman as $p)
                                    <option value="{{ $p->id_peminjaman }}"> {{ $p->buku->kode_buku }} -
                                        {{ $p->buku->judul }} -
                                        {{ $p->tanggal_pinjam }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-2">
